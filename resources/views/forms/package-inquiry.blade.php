...
<title>@lang('titles.package', ['package' => $object['title_'.$locale]]) - Online @lang('titles.inquiry') - Belgrade Luxury</title>
<meta name="description" content="{{ $object['description_'.$locale] }}" />
<!-- Facebook share meta tags -->
<meta property="og:title" content="{{ Lang::get('titles.package', ['package' => $object['title_'.$locale]]) }} - Online {{ Lang::get('titles.inquiry') }} - Belgrade Luxury" />
<meta property="og:description" content="{{ $object['description_'.$locale] }}" />
<meta property="og:image" content="{{ asset('storage/images/'.$object->cardFront) }}" />   
@stop
    ...
    <a rel="alternate" hreflang="{{ $localeCode }}" 
       href="
        <?php 
            ...
            $titleEnd = strpos($url, "-");
            $url = substr($url, 0, $titleStart + 1).str_replace(' ', '-', $object['title_'.$localeCode]).substr($url, $titleEnd);
            echo $url;
        ?>
       ">
        {{ $properties['native'] }}
...
@section('content')
<!--    START FORM SECTION      -->
<section id="form" class="contact-section panel fullwidth background-properties space-y" data-section-name="form-panel" style="background-image: url({{ asset('storage/images/'.$object->cardFront) }})">
            ...
            <div>
                ...
                <div class="description">                                      
                    <div class="text-uppercase">
                        <a class="hi-icon "
                           href="{{ route("package", [ 'title' => str_replace(" ", "-", $object['title_'.$locale]) ]) }}"
                           style="background-image: url({{ asset('storage/images/'.$object->symbol) }}); background-size: cover; background-position: center center"></a>
                    </div>
                </div>
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="form-group">
                            <label for="object" class="col-xs-4 col-sm-offset-0 col-sm-3 control-label">@lang('forms.package'):</label>

                            <div class="col-xs-8 col-sm-9" style="padding-right: 30px">
                                <select id="object" class="form-control text-capitalize" name="object">
                                    
                                    @foreach($objects as $o)  
                                    <option value="{{ $o->packID }}"
                                            <?php
                                        if (isset($object)) {
                                            if ($object->packID === $o->packID) {
                                                echo "selected";
                                            }
                                        } 
                                        ?>
